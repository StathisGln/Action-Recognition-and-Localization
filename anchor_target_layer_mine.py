...
from bbox_transform import clip_boxes, bbox_overlaps_time, bbox_transform_batch
import pdb
...
class _AnchorTargetLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        gt_boxes = input[1]      ## gt tube
        im_info = input[2]       ## im_info
        num_boxes = input[3]     ## number of gt_boxes 
        time_limit = input[4]
        # map of shape (..., H, W)
        ...
        print('feat_height {}, feat_width {}'.format(feat_height, feat_width))
        ...
        # print('inds_inside.shape :',inds_inside.shape)

        # # keep only inside anchors
        # anchors = all_anchors[inds_inside, :]
        # all_anchors = anchors

        # keep =  ( (all_anchors[:, 2] < long(im_info[0][1]) + self._allowed_border) &
        ...
        overlaps = bbox_overlaps_time(anchors, gt_boxes, time_limit)
        ...
        for i in range(batch_size):
            ...
            print('now we have bg samples :', num_bg)
        ...
        # print(gt_boxes.view(-1,7)[argmax_overlaps.view(-1), :].view(batch_size, -1, 7)) 
        # print(gt_boxes.view(-1,7)[argmax_overlaps.view(-1), :].view(batch_size, -1, 7).shape)
