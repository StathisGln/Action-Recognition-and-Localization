...
class _AnchorTargetLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        bbox_targets = bbox_targets.view(batch_size, time, -1, self.time_dim[0]*4).contiguous()
        outputs.append(bbox_targets)

        anchors_count = bbox_inside_weights.size(1)
        bbox_inside_weights = bbox_inside_weights.view(batch_size,anchors_count,1).contiguous().view(batch_size,time,-1,1).contiguous()
        ...
        bbox_outside_weights = bbox_outside_weights.view(batch_size,anchors_count,1).contiguous().view(batch_size,time,-1,1).contiguous()
