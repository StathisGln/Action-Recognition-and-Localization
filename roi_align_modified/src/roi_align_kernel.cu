...
#define CUDA_1D_KERNEL_LOOP(i, n)                            \
    ...
    __global__ void ROIAlignForward(const int nthreads, const float* bottom_data, const float spatial_scale, const int height, const int width,
	    ...
	    // create little bigger rois
	    float mod_roi_start_w = roi_start_w - 0.5;
	    float mod_roi_start_h = roi_start_h - 0.5;
	    float mod_roi_end_w   = roi_end_w   + 0.5;
	    float mod_roi_end_h   = roi_end_h   + 0.5;

	    if (mod_roi_start_w < 0)
	      mod_roi_start_w = 0;
	    if (mod_roi_start_h < 0)
	      mod_roi_start_h = 0;

	    if (mod_roi_end_w > width)
	      mod_roi_start_w = width;
	    if (mod_roi_end_h > height)
	      mod_roi_start_h = height;

	    // if (index == 0){
	    //   printf("height: %d  width %d\n", height, width);
	    //   printf("mod_roi_start_w %f, mod_roi_start_h %f, mod_roi_end_w %f, mod_roi_end_h %f\n",
	    // 	     mod_roi_start_w, mod_roi_start_h, mod_roi_end_w,mod_roi_end_h);
	    //   printf("roi_start_w %f, roi_start_h %f, roi_end_w %f, roi_end_h %f\n",
	    // 	     roi_start_w, roi_start_h, roi_end_w, roi_end_h);
	    //   printf("bottom_rois[n * 5 + 1] %f %f %f  %f\n",
	    // 	     bottom_rois[n * 5 + 1], bottom_rois[n * 5 + 2],bottom_rois[n * 5 + 3],bottom_rois[n * 5 + 4]);
	    // }
            ...
            // The same with mod rois
            float mod_roi_width  = fmaxf(mod_roi_end_w - mod_roi_start_w + 1., 0.);
            float mod_roi_height = fmaxf(mod_roi_end_h - mod_roi_start_h + 1., 0.);
            float mod_bin_size_h = mod_roi_height / (aligned_height - 1.);
            float mod_bin_size_w = mod_roi_width / (aligned_width - 1.);
	    ...
	    float mod_h = (float)(ph) * mod_bin_size_h + mod_roi_start_h;
            float mod_w = (float)(pw) * mod_bin_size_w + mod_roi_start_w;
	    int mod_hstart = fminf(floor(mod_h), height - 2);
            int mod_wstart = fminf(floor(mod_w), width - 2);
	      ...
	      if (h < 0 || h >= height || w < 0 || w >= width) { // if mod rois doesnt fit in
                ...
                float h_ratio = mod_h - (float)(mod_hstart);
                float w_ratio = mod_w - (float)(mod_wstart);
                int upleft = img_start + (c * height + mod_hstart) * width + mod_wstart;
                int upright = upleft + 1;
                int downleft = upleft + width;
                int downright = downleft + 1;
