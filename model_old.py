...
class Model(nn.Module):
    ...
    def __init__(self, actions, sample_duration, sample_size):
        ...
        self.p_feat_size = 256 # 128 # 256 # 512        
        # For connection 
    ...
    def forward(self,n_devs, dataset_folder, vid_names, clips, vid_id, boxes, mode, cls2idx, num_actions, num_frames, h_, w_):
        ...
        for step, dt in enumerate(data_loader):
            ...
            for i in range(idx_s, idx_e):
                ...
                # overlaps_ = tube_overlaps(p_tubes[i-1,:,6*4:7*4],p_tubes[i,:,:1*4]).type_as(p_tubes)
                overlaps_ = tube_overlaps(p_tubes[i-1,:,7*4:],p_tubes[i,:,:1*4]).type_as(p_tubes)
                pos, pos_indices, \
        ...
        for i in range(final_poss.size(0)):
            ...
            for j in range(final_poss.size(1)):
                ...
                start_fr = curr_[0]* self.step
                end_fr = min((curr_[0]*self.step+self.sample_duration).type_as(num_frames), num_frames).type_as(start_fr)
        ...
        # final_feats = []
        f_feats = torch.zeros(len(f_tubes),n_clips, self.p_feat_size, self.sample_duration).type_as(features) - 1
