...
class Model(nn.Module):
    ...
    def forward(self,n_devs, dataset_folder, vid_names, clips, vid_id, boxes, mode, cls2idx, num_actions, num_frames, h_, w_, target):
        ...
        after_nms_tubes = torch.zeros(batch_size)
        ...
        for b in range(batch_size):
            ...
            # print('fin_tubes.shape :',fin_tubes.shape)
            # print('final_scores.unsqueeze(1).shape :',final_scores.unsqueeze(1).shape)
            # print('tubes_trsl.shape :',tubes_trsl.shape)
            ...
            after_nms_tubes[b] = keep_idx_i.numel()
            # ###
        ...
        for b in range(batch_size):
            ...
            for i in range(after_nms_tubes[b].int()):

                t = final_combinations[b,i].ne(-1).all(dim=1).nonzero().view(-1)
                if t.numel() < 1:
                    continue
                indices = final_combinations[b,i,t].long()
                feat_rnn = features[b,indices[:,0],indices[:,1]].max(dim=2)[0]
                ret = self.act_rnn(feat_rnn.view(feat_rnn.size(0),-1))
                prob_out[b,i] = self.act_rnn(feat_rnn.view(feat_rnn.size(0),-1))
        ...
        if self.training:
            ...
            # ret_tubes = torch.zeros(1,self.post_nms_tubes, ret_n_frames,4).type_as(final_tubes).float() -1
            # ret_prob_out = torch.zeros(1,self.post_nms_tubes,self.n_classes).type_as(final_tubes).float() - 1
            ...
            return final_tubes, prob_out, torch.Tensor([final_tubes.size(0)]).cuda()
    ...
    def load_part_model(self, action_model_path=None, rnn_path=None):
        ...
        # load lstm
        if rnn_path != None:
            print('rnn_path :',rnn_path)
            act_rnn = Act_RNN(self.p_feat_size*self.POOLING_SIZE*self.POOLING_SIZE,int(self.p_feat_size/2),self.n_classes)
            ...
            self.act_rnn =Act_RNN(self.p_feat_size*self.POOLING_SIZE*self.POOLING_SIZE,int(self.p_feat_size/2),self.n_classes) 
