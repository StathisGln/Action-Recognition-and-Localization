...
def validation(epoch, device, model, dataset_folder, sample_duration, spatial_transform, temporal_transform, boxes_file, splt_txt_path, cls2idx, batch_size, n_threads):
    ...
    for step, data  in enumerate(data_loader):
        ...
        tubes, _, _, _, _, _,r_prog, _, cls_score, _, _, \
...
def training(epoch, device, model, dataset_folder, sample_duration, spatial_transform, temporal_transform, boxes_file, splt_txt_path, cls2idx, batch_size, n_threads, lr, mode = 1):
    ...
    for step, data  in enumerate(data_loader):
        ...
        rois_rate,\
        rois_rate_loss, rois_prog, rois_prog_loss, cls,\
        cls_loss, rois_label, \
        ...
        if mode == 3:
            ...
            loss = sgl_rois_bbox_loss.mean() + rois_rate_loss.mean() + cls_loss.mean() + rois_prog_loss.mean()
...
if __name__ == '__main__':
    ...
    #     # if ( epoch + 1 ) % 5 == 0:
    #     #     torch.save(act_model.state_dict(), "action_net_model_jhmdb_16frm_64_cls.pwf".format(epoch+1))
    ...
    model_data = torch.load('./action_net_model_jhmdb_16frm_64_cls_sep.pwf')
    ...
    for p in act_model.module.prg_rt.parameters() : p.requires_grad=True
    ...
    for epoch in range(epochs):
        ...
        act_model, loss = training(epoch, device, act_model, dataset_folder, sample_duration, spatial_transform, temporal_transform, boxes_file, split_txt_path, cls2idx, n_devs*4, 0, lr, mode=7)
        if ( epoch + 1 ) % 5 == 0:
            torch.save(act_model.state_dict(), "action_net_model_jhmdb_16frm_64_cls_sep_2.pwf".format(epoch+1))
    ...
    torch.save(act_model.state_dict(), "action_net_model_jhmdb_16frm_64_cls_sep_2.pwf")
