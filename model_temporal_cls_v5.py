...
from soft_nms_3d.py_nms import py_cpu_softnms, py_cpu_temp_softnms 
import time 
...
class Model(nn.Module):
    ...
    def forward(self,n_devs, dataset_folder, vid_names, clips, vid_id, boxes, mode, cls2idx, num_actions, num_frames, h_, w_, target):
        ...
        for b in range(batch_size):
            ...
            f_scores_limits_all = f_scores_limits[keep_idx_i].contiguous()

            # use nms to remove all double tubes
            print('f_tubes_limits :',f_tubes_limits.cpu().numpy())
            print('f_scores_limits :',f_scores_limits_all.cpu().numpy())
            print('f_tubes_limits :',f_tubes_limits[:16].cpu().numpy())
            print('f_scores_limits :',f_scores_limits_all[:16].cpu().numpy())
            exit(-1)
            keep_idx_i = py_cpu_temp_nms(torch.cat([f_tubes_limits,f_scores_limits.unsqueeze(1)], dim=1), 0.1)
            keep_idx_i = torch.Tensor(keep_idx_i).cuda().long().view(-1)
            ...
            f_scores_limits = torch.zeros(n_temp_tubes).type_as(f_tubes_limits)
            f_cls_pred = torch.zeros(n_temp_tubes).type_as(f_tubes_limits)
            ...
            for i in range(n_temp_tubes):
                ...
                if indices_vid.size(0) <= max_num_videos:
                    ...
                    with torch.no_grad():
                        ...
                        ret, _ = self.resnet(clips_.permute(0,2,1,3,4),None,None)
                    ...
                    for j in range(0,indices_vid.size(0),max_num_videos):
                        ...
                        with torch.no_grad():
                            ...
                            ret[j:j+max_num_videos], _ = self.resnet(clips_.permute(0,2,1,3,4), None, None)
                            # ret[j:j+max_num_videos]    = F.softmax(ret[j:j+max_num_videos],dim=1)
                ret_score = ret.mean(dim=0)
                # ret_score = F.softmax(ret_score, dim=0)
                ...
                f_scores_limits[i] = max_score
                f_cls_pred[i] = max_indices

            print('f_scores_limits.shape :',f_scores_limits.shape)
            confidence_scores[b, :f_tubes_limits.size(0)] = f_scores_limits
            cls_pred[b,:f_tubes_limits.size(0)] = f_cls_pred
            ret_tubes[b,:f_tubes_limits.size(0)] = f_tubes_limits

            # keep_idx_i = py_cpu_temp_nms(torch.cat([f_tubes_limits,f_scores_limits.unsqueeze(1)], dim=1), 0.1)
            # keep_idx_i = torch.Tensor(keep_idx_i).cuda().long().view(-1)
            # confidence_scores[b, :keep_idx_i.numel()] = f_scores_limits[keep_idx_i]
            # cls_pred[b,:keep_idx_i.numel()] = f_cls_pred[keep_idx_i]
            # ret_tubes[b,:keep_idx_i.numel()] = f_tubes_limits[keep_idx_i]
    ...
    def load_part_model(self, action_model_path=None, rnn_path=None):
        ...
        if rnn_path != None:
            ...
            act_model = ACT_cls(self.classes, self.sample_duration)
            act_model.create_architecture()
            act_model.load_state_dict(new_state_dict)
            self.resnet = act_model

        else:

            self.resnet = ACT_cls(self.classes, self.sample_duration)
            self.resnet.create_architecture()
