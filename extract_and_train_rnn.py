...
from ucf_dataset import  video_names, RNN_UCF
...
def validation(epoch, device, model, dataset_folder, sample_duration, spatial_transform, temporal_transform, boxes_file, splt_txt_path, cls2idx, batch_size, n_threads):
    ...
    file_p = open('validation_ucf_linear.txt', 'a')
    file_p.write(' -----------------------\n')
...
if __name__ == '__main__':
    ...
    # out_path = 'UCF-101-features-256-7-32-8'
    out_path = 'UCF-101-features-16-256-7-32-82'
    mode = 'extract'
    for step, data  in enumerate(data_loader):
        ...
        ### TO REMOVE
            ...
            try:
                print('Folder exists, trying to read from \'feats.pt\' file')
                feats = torch.load(os.path.join(out_path, vid_names[vid_id],'feats.pt'))
                print('feats.shape :',feats.shape)
                print('feats.dim :',feats.dim())
                if feats.size(0) == 36 and feats.dim() == 5:
                    print('Ok this file...')
                    continue
            except:
