...
class Model(nn.Module):
    ...
    def __init__(self, actions, sample_duration, sample_size, ):
        ...
        print('self.temp_num_nms_tubes :',self.temp_num_nms_tubes)
    ...
    def forward(self,n_devs, dataset_folder, vid_names, clips, vid_id, boxes, mode, cls2idx, num_actions, num_frames, h_, w_, target):
        ...
        for b in range(batch_size):
            ...
            print('f_tubes_limits.shape :',f_tubes_limits.shape)
            if f_scores_limits.size(0) > self.temp_num_nms_tubes:
                _, max_score_indices = torch.topk(f_scores_limits, self.temp_num_nms_tubes)                

                f_scores_limits = f_scores_limits[max_score_indices]
                f_tubes_limits = f_tubes_limits[max_score_indices]
            print('f_scores_limits.shape :',f_scores_limits.shape)
            print('f_tubes_limits.shape :',f_tubes_limits.shape)
            ## temp nms again
            # keep_idx_i = py_cpu_temp_nms(torch.cat([f_tubes_limits,f_scores_limits.unsqueeze(1)], dim=1), 0.9)
            # keep_idx_i = py_cpu_temp_nms(torch.cat([f_tubes_limits,f_scores_limits.unsqueeze(1)], dim=1), 0.5)
