...
class ResNet_rest(nn.Module):

    def __init__(self, block, layers, sample_size, sample_duration, shortcut_type='B', num_classes=400, last_fc=True):
        self.last_fc = last_fc

        self.inplanes = 64
        super(ResNet_rest, self).__init__()
        self.conv1 = nn.Conv3d(3, 64, kernel_size=7, stride=(1, 2, 2),
                               padding=(3, 3, 3), bias=False)
        self.bn1 = nn.BatchNorm3d(64)
        self.relu = nn.ReLU(inplace=True)
...
def resnet34_rest(**kwargs):
    """Constructs a ResNet-34 model.
    """
    model = ResNet_rest(BasicBlock, [3, 4, 6, 3], **kwargs)
    return model

def resnet34_orig(**kwargs):
    """Constructs a ResNet-34 model.
    """
    model = ResNet_orig(BasicBlock, [3, 4, 6, 3], **kwargs)
    return model
