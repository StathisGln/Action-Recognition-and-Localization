...
class Model(nn.Module):
    ...
    def forward(self,n_devs, dataset_folder, vid_names, clips, vid_id, boxes, mode, cls2idx, num_actions, num_frames, h_, w_, target):
        ...
        #     # k_epil = min(self.,final_scores.size(0))
        #     # _, indices = torch.topk(final_scores,k_epil)
        ...
        #     # print('fianl_scores.shape :',final_scores.shape)
        #     ### NMS
        ...
        #     keep_idx_i = py_cpu_nms_tubes(torch.cat([fin_tubes,final_scores.unsqueeze(1)], dim=1).cpu().numpy(), 0.7)
        ...
        if mode == 'noclass':
            ...
            ret_final_tubes = torch.zeros(batch_size,self.post_nms_tubes,900,4).cuda()
