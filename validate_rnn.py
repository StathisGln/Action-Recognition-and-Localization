...
from model_par_transl_v3_rnn import Model
# from model_all_rnn_nms import Model
...
def validation(epoch, device, model, dataset_folder, sample_duration, spatial_transform, temporal_transform, boxes_file, splt_txt_path, cls2idx, batch_size, n_threads):
    ...
    for step, data  in enumerate(data_loader):
        ...
        for i in range(batch_size):
            ...
            f_tubes_ = f_tubes[keep_indices].contiguous()
            if f_tubes_.size(0) > 10:

                _,sort_idx = torch.topk(f_tubes_[:,1],10)
                f_tubes_ = f_tubes_[sort_idx]
            
            detection_dic[v_name] = f_tubes_.contiguous().float()
            ...
            if f_tubes_.size(0) > 10:
                _,sort_idx = torch.topk(f_tubes_[:,1],10)
                f_tubes_ = f_tubes_[sort_idx]
            detection_dic_75[v_name] = f_tubes_.contiguous().float()
            ...
            if f_tubes_.size(0) > 10:
                _,sort_idx = torch.topk(f_tubes_[:,1],10)
                f_tubes_ = f_tubes_[sort_idx]
            detection_dic_3[v_name] = f_tubes_.contiguous().float()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--linear_path', '-l', help='Path of linear model', default='./linear_ucf_60.pwf' )
