...
class Model(nn.Module):
    ...
    def forward(self,n_devs, dataset_folder, vid_names, clips, vid_id, boxes, mode, cls2idx, num_actions, num_frames, h_, w_, target):
        ...
        for b in range(batch_size):
            ...
            f_features = features.new(keep_indices_batch[b],  n_filters, \
            ...
            for i in range(keep_indices_batch[b]):
                ...
                f_features[i]  = features[b,indices[:,0],indices[:,1]].mean(0)
                # feat_rnn = features[b,indices[:,0],indices[:,1]].mean(0).unsqueeze(0)
                # feat_rnn = features[indices[:,0],indices[:,1]].max(0)[0].unsqueeze(0)            

            prob_out[b,:keep_indices_batch[b]] = self.act_rnn(f_features.view(keep_indices_batch[b],-1))
    ...
    def load_part_model(self, action_model_path=None, rnn_path=None):
        ...
        if rnn_path != None:
            ...
            act_rnn = nn.Sequential(
                ...
                nn.Linear(self.p_feat_size*self.POOLING_SIZE*self.POOLING_SIZE, self.n_classes),
