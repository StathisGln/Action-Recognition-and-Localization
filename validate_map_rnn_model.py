...
def validation(epoch, device, model, dataset_folder, sample_duration, spatial_transform, temporal_transform, boxes_file, splt_txt_path, cls2idx, batch_size, n_threads):
    ...
    n_tubes = conf.MODEL_POST_NMS_TUBES
    ## 2 rois : 1450
    tubes_sum = 0
    for step, data  in enumerate(data_loader):
        ...
        for i in range(batch_size):
            ...
            print('boxes[i,:,:n_frames[i],:4].shape :',boxes[i,:,:n_frames[i],:4].shape)
            print('boxes[i,:,:n_frames[i],:4].shape :',boxes[i,:n_actions[i].long(),:n_frames[i].long(),:4].shape)
            print('boxes[i,:,:n_frames[i],:4].contiguous().view(n_frames[i]*4).shape ',\
                  boxes[i,n_actions[i].long(),:n_frames[i].long(),:4].contiguous().view(n_frames[i]*4).shape)
            f_boxes = torch.cat([target[i].unsqueeze(0).type_as(boxes),\
                                 boxes[i,:n_actions[i].long(),:n_frames[i].long(),:4].contiguous().view(n_frames[i]*4)]).unsqueeze(0)
