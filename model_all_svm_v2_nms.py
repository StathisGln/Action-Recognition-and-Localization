...
class Model(nn.Module):
    ...
    def forward(self,n_devs, dataset_folder, vid_names, clips, vid_id, boxes, mode, cls2idx, num_actions, num_frames, h_, w_, target):
        ...
        for i in range(final_combinations.size(0)):
            ...
            # f_features.append( self.temporal_pool(features[indices[:,0],indices[:,1]]))
            # f_features.append( self.temporal_pool(features[indices[:,0],indices[:,1]]).mean(2))
            # f_features.append( self.temporal_pool(features[indices[:,0],indices[:,1]]).max(2)[0])
            f_features.append( self.temporal_pool(features[indices[:,0],indices[:,1]]))

            # f_features.append( self.temporal_pool(features[indices[:,0],indices[:,1]]).max(2)[0])
            # f_features.append( features[indices[:,0],indices[:,1]].max(0)[0])
            # f_features.append( features[indices[:,0],indices[:,1]].mean(0))
        f_features = torch.stack(f_features).type_as(clips)
        # f_features = torch.stack(f_features)
        ...
        scores = torch.mm( f_features.view(f_features.size(0),-1) * 1.0 / self.svmFeatScale.type_as(f_features),\
                           self.svmWeights.type_as(f_features).t()) + self.svmBias.type_as(f_features)
        scores = scores.cuda()
