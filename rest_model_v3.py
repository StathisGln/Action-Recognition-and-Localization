...
class RestNet(nn.Module):
    ...
    def __init__(self, actions,sample_duration, pooling_time):
        ...
        self.head_to_tail_ =  torch.nn.Sequential(  # 
            nn.Linear(512 * self.sample_duration * 4 * 4, 1024),   # change from 4096 to 2048, for memory limit
            nn.ReLU(True),
            nn.Dropout(),
    ...
    def forward(self, features, combinations,  target, mode):
        ...
        f_features = features.new(batch_size, rois_per_image, self.pooling_time, n_filters, self.sample_duration, \
                                 x_axis, y_axis).zero_().type_as(features)
        ...
        for b in range(batch_size):
            ...
            for i in range(rois_per_image):
                ...
                f_features[b,i]  = self.temporal_pool(features[b,indices[:,0],indices[:,1]])
                # f_features[b,i]  = self.temporal_pool(features[b,indices[:,0],indices[:,1]]).max(2)[0]

        f_features = f_features.permute(0,1,3,2,4,5,6)
        feats = self.top_net(f_features.contiguous().view(batch_size*rois_per_image,\
                                             n_filters, self.pooling_time*self.sample_duration, x_axis,y_axis))
        ...
        feats = self.head_to_tail_(feats.view(batch_size*rois_per_image,-1))
