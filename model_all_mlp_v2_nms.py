...
class Model(nn.Module):
    ...
    def __init__(self, actions, sample_duration, sample_size, ):
        ...
        print('self.post_nms_topN :',self.post_nms_tubes)
        ...
        self.calc = Calculator(self.max_num_tubes, self.update_thresh, self.connection_thresh, self.pre_nms_tubes)
    ...
    def forward(self,n_devs, dataset_folder, vid_names, clips, vid_id, boxes, mode, cls2idx, num_actions, num_frames, h_, w_, target):
        ...
        keep_idx_i = py_cpu_nms_tubes(torch.cat([fin_tubes,conn_scores.unsqueeze(1)], dim=1), 0.7)
        ...
        prob_out = torch.zeros( self.post_nms_tubes,self.n_classes).type_as(clips)

        n_filters = features.size(-4)
        x_axis = features.size(-2)
        y_axis = features.size(-1)

        # f_features = features.new(final_combinations.size(0), self.pooling_time, n_filters, self.sample_duration,\
        #                           x_axis, y_axis).zero_().type_as(features)
        f_features = features.new(final_combinations.size(0), self.pooling_time, n_filters, \
                                  x_axis, y_axis).zero_().type_as(features)
        ...
        for i in range(final_combinations.size(0)):
            ...
            f_features[i]  = self.temporal_pool(features[indices[:,0],indices[:,1]]).max(2)[0]

        f_features = f_features.permute(0,2,1,3,4)
        f_features = self.act_rnn_top(f_features).mean(4).mean(3)
        prob_out = self.cls(f_features.view(f_features.size(0),-1))
        ...
        if self.training:
            ...
            if final_tubes.size(0) > conf.MODEL_POST_NMS_TUBES:
                ...
                final_tubes = final_tubes[indices[:conf.MODEL_POST_NMS_TUBES]].contiguous()
                prob_out = prob_out[indices[:conf.MODEL_POST_NMS_TUBES]].contiguous()

            ret_tubes = torch.zeros(1,conf.MODEL_POST_NMS_TUBES, ret_n_frames,4).type_as(final_tubes).float() -1
            ret_prob_out = torch.zeros(1,conf.MODEL_POST_NMS_TUBES,self.n_classes).type_as(final_tubes).float() - 1
    ...
    def load_part_model(self, action_model_path=None, rnn_path=None):
        ...
        if rnn_path != None:
            ...
            act_rnn =  torch.nn.Sequential(  # 
                nn.Linear(256 * self.sample_duration * 7 * 7, 1024),   # change from 4096 to 2048, for memory limit
                nn.ReLU(True),
                nn.Dropout(),
            ...
            act_rnn = RestNet(self.n_classes, self.sample_duration, pooling_time=2)
            ...
            # self.temporal_pool = act_rnn.temporal_pool
