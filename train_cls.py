...
def validation(epoch, device, model, dataset_folder, sample_duration, spatial_transform, temporal_transform, boxes_file, splt_txt_path, cls2idx, batch_size, n_threads):
    ...
    for step, data  in enumerate(data_loader):
        ...
        clips, h, w,  n_frames, im_info,target = data
        clips_ = clips.to(device)
        im_info_ = im_info.to(device)
        target_ = target.to(device)

        cls_scr, cls_loss  = model(clips_, \
                                   im_info_,
                                   target_)

        _,cls_ints = torch.max(cls_scr, dim=1)

        equal = cls_ints.long() == target_.long()
        correct = equal.sum()

        correct_preds += correct.numel()
        all_preds += equal.numel()
        
    print(' -----------------------')
    print('|                       |')
    print('| Validation Epoch: {: >3} | '.format(epoch+1))
    print('|                       |')
    print('| Correct         {: >5} |'.format(correct_preds))
    print('|                       |')
    print('| Out of          {: >5} |'.format(all_preds))
    print('|                       |')
    print(' -----------------------')
...
if __name__ == '__main__':
    ...
    # Init action_net
    act_model = ACT_cls(actions, sample_duration)
    ...
    for epoch in range(epochs):
        ...
        if ( epoch + 1 ) % 5 == 0:
            torch.save(act_model.state_dict(), "action_net_cls_ucf_16frm.pwf".format(epoch+1))
    ...
    torch.save(act_model.state_dict(), "action_net_cls_ucf_16frm.pwf")
