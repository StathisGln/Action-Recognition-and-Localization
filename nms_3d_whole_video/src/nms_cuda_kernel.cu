...
__device__ inline float devIoU(float const * const a, float const * const b, const int n_frames) {
  ...
  for (int i=0; i<T; i++){
    ...
    top = max(a[1 + i*4], b[1 + i*4]), bottom = min(a[3 + i*4], b[3 + i*4]);
...
__global__ void nms_kernel(int n_boxes, float nms_overlap_thresh, int n_frames,
  ...
  const int T = 40; // max num of frames
  ...
  if (threadIdx.x < row_size) {
    ...
    if (row_start == col_start) {
      // printf("row_start == col_start  %d %d\n", row_start, col_start);
      start = threadIdx.x + 1;
    }
