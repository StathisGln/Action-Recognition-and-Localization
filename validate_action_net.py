...
def validation(epoch, device, model, dataset_folder, sample_duration, spatial_transform, temporal_transform, boxes_file, splt_txt_path, cls2idx, batch_size, n_threads):
    ...
    for step, data  in enumerate(data_loader):
        ...
        for i in range(tubes.size(0)): # how many frames we have
            ...
            # print('tubes_exp_.shape :',tubes_exp_.shape)
            ...
            for z in range(boxes_.size(0)):

                non_empty_frames_idx = boxes_[z,:,4].nonzero()
                sgl_detected =  gt_max_overlaps_sgl[non_empty_frames_idx, z].ne(0).sum()
                n_elems = gt_max_overlaps_sgl[non_empty_frames_idx, z].nelement()
...
if __name__ == '__main__':
    ...
    # model_data = torch.load('./action_net_model_block2.pwf')
