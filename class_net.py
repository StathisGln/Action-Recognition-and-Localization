...
from human_reg_newedw_multi import _Regression_Layer
...
class ACT_net(nn.Module):
    ...
    def __init__(self, actions,sample_duration):
        ...
        self.temp_scale = 1.0
        self.reg_layer = _Regression_Layer(64, self.sample_duration, self.n_classes).cuda()
        ...
        self.avgpool = nn.AvgPool3d(
            (self.sample_duration, 4, 4), stride=1)
    ...
    def forward(self, im_data, im_info, gt_tubes, gt_rois,  start_fr):
        ...
        base_feat_3 = self.act_base_3(base_feat_2).mean(4).mean(3).mean(2)
        cls_score   = self.cls(base_feat_3)
        ...
        if self.training :
            ...
            sgl_rois_bbox_pred_view = sgl_rois_bbox_pred.view(sgl_rois_bbox_pred.size(0), int(sgl_rois_bbox_pred.size(1) / (self.sample_duration* 4)), 4*self.sample_duration)
            sgl_rois_bbox_pred_select = torch.gather(sgl_rois_bbox_pred_view, 1, rois_label.view(rois_label.size(0), 1, 1).expand(rois_label.size(0), 1, 4*self.sample_duration))
            sgl_rois_bbox_pred = sgl_rois_bbox_pred_select.squeeze(1)
    ...
    def _init_modules(self):
        ...
        self.act_base_3 = nn.Sequential(model.module.layer4)
        # Fix blocks
