...
def imresizeAndPad(img, width, height, pad_value=114):
    ...
    resizedWidth, resizedHeight = imgResized.shape[1], imgResized.shape[0]
...
class Scale(object):
    ...
    def __call__(self, img):
        ...
        if isinstance(self.size, int):
            w, h = img.size
            return imresizeAndPad(img, self.size, self.size)
        else:
            return imresizeAndPad(img, self.size[0], self.size[1])
