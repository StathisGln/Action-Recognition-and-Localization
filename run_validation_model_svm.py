...
def validation(epoch, device, model, dataset_folder, sample_duration, spatial_transform, temporal_transform, boxes_file, splt_txt_path, cls2idx, batch_size, n_threads):
    ...
    vid_id, clips, boxes, n_frames, n_actions, h, w, target, clips_plot =vid_name_loader[10]
    vid_id = torch.from_numpy(vid_id).unsqueeze(0).int()
    print('vid_id :',vid_id)
    ...
    n_frames = torch.Tensor([n_frames]).int().to(device)
    n_actions = torch.Tensor([n_actions]).unsqueeze(0).int().to(device)
    target = torch.Tensor([target]).to(device)

    h = torch.Tensor(h).long()
    w = torch.Tensor(w).long()
    im_info = torch.cat([h,w,torch.ones(clips.size(0)).long()*clips.size(2)]).unsqueeze(0).to(device)
    mode = 'test'
    # print('target :',target)
    ...
    for i in range(batch_size):
        ...
        print('prob[i,:2] :',prob_out[i,:2])
        print('cls_int :',cls_int[:2])
        ...
        print('cls_int.view(-1,1) :',cls_int.view(-1,1).shape)
        print('f_prob.shape :',f_prob.shape)
        print('f_prob.view(-1,1).shape :',f_prob.view(-1,1).shape)
        print('tubes[i,:n_tubes[i].long(), :n_frames[i]].view(-1,n_frames[i]*4) :',\
              tubes[i,:n_tubes[i].long(), :n_frames[i]].view(-1,n_frames[i]*4).shape)
        ...
        print('target.shape :',target.shape)
        print('boxes[i,:,:n_frames[i],:4].contiguous().view(n_frames[i]*)4 :',\
              boxes[i,:,:n_frames[i],:4].contiguous().view(n_frames[i]*4).shape)
