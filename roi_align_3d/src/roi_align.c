...
void ROIAlignForwardCpu(const float* bottom_data, const float spatial_scale, const int num_rois,
			const int height, const int width, const int time, const int channels,
			const int aligned_height, const int aligned_width, const int time_dim,
...
int roi_align_forward(int aligned_height, int aligned_width, int time_dim, float spatial_scale,
                     THFloatTensor * features, THFloatTensor * rois, THFloatTensor * output)
{
    ...
    // do ROIAlignForward
    ROIAlignForwardCpu(data_flat, spatial_scale, num_rois, data_height, data_width, data_time, num_channels,
            aligned_height, aligned_width, rois_flat, output_flat);

    return 1;
}

int roi_align_backward(int aligned_height, int aligned_width, int time_dim, float spatial_scale,
                       THFloatTensor * top_grad, THFloatTensor * rois, THFloatTensor * bottom_grad)
{
    ...
    ROIAlignBackwardCpu(top_grad_flat, spatial_scale, num_rois, data_height,
			data_width, data_time, num_channels, aligned_height, aligned_width, rois_flat, bottom_grad_flat);
			...
			float* top_data)
{
	...
	int t = (idx / aligned_width / aligned_height) % time_dim;
	...
	float roi_start_t = bottom_rois[n * 7 + 3] * 1;
        float roi_end_w = bottom_rois[n * 7 + 4] * spatial_scale;
        float roi_end_h = bottom_rois[n * 7 + 5] * spatial_scale;
	float roi_end_t = bottom_rois[n * 7 + 6] * 1;

        // Force malformed ROI to be 1x1
        float roi_width = fmaxf(roi_end_w - roi_start_w + 1., 0.);  // fmaxf finds the max value
	...
	float roi_time = fmaxf(roi_end_t - roi_start_t + 1., 0.);
        ...
        float bin_size_t = roi_time / (time_dim - 1.); // not sure about that

        float h = (float)(ph) * bin_size_h + roi_start_h;
        float w = (float)(pw) * bin_size_w + roi_start_w;
	float t = (float)(t)  * bin_size_t + roi_start_t;
