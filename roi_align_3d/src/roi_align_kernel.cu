...
#define CUDA_1D_KERNEL_LOOP(i, n)                            \
  ...
  __global__ void ROIAlignForward(const int nthreads, const float* bottom_data, const float spatial_scale, const int time, const int height, const int width,
	    ...
	    int t  = (index / aligned_width / aligned_height) % time;
            int c  = (index / aligned_width / aligned_height / time ) % channels;
            int n  = index / aligned_width / aligned_height / time / channels;
  ...
  int ROIAlignForwardLaucher(const float* bottom_data, const float spatial_scale, const int num_rois, const int data_time, const int height, const int width,
        ...
        ROIAlignForward<<<(output_size + kThreadsPerBlock - 1) / kThreadsPerBlock, kThreadsPerBlock, 0, stream>>>(
														  output_size, bottom_data, spatial_scale, time, height, width, channels,
