...
def resize_tube(gt_rois, h_tensor,w_tensor, sample_size):
    ...
    for i in range(batch_size):
        ...
        gt_rois[i,:,0] = (( gt_rois[i,:,0]-left ) * sample_size/w).clamp_(min=0)
        gt_rois[i,:,1] = (( gt_rois[i,:,1]-top  ) * sample_size/h).clamp_(min=0)
        gt_rois[i,:,3] = (( gt_rois[i,:,3]-left ) * sample_size/w).clamp_(min=0)
        gt_rois[i,:,4] = (( gt_rois[i,:,4]-top  ) * sample_size/h).clamp_(min=0)
