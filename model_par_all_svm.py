...
class Model(nn.Module):
    ...
    def forward(self,n_devs, dataset_folder, vid_names, clips, vid_id, boxes, mode, cls2idx, num_actions, num_frames, h_, w_, target):
        ...
        cls_scr, cls_loss = self.restNet(features, final_combinations, tubes_labels, mode='extract')
        ...
        scores = torch.zeros(batch_size, final_feats.size(1),self.n_classes).type_as(final_feats)
        for b in range(batch_size):
            scores[b] = scoreTubes(final_feats[b],\
