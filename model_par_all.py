...
class Model(nn.Module):
    ...
    def __init__(self, actions, sample_duration, sample_size, ):
        ...
        self.post_nms_tubes = 10
        # self.post_nms_tubes = conf.MODEL_POST_NMS_TUBES
        self.n_ret_tubes = 2000 if self.training else conf.ALL_SCORES_THRESH
    ...
    def forward(self,n_devs, dataset_folder, vid_names, clips, vid_id, boxes, mode, cls2idx, num_actions, num_frames, h_, w_, target):
        ...
        #         f_tubes[b] = final_tubes[keep_idx_i]
        ...
        for b in range(batch_size):
            ...
            first_frm = tub_limits.min().int()
            last_frm = tub_limits.max().int()
            ...
            indices = indices + frm_indices.unsqueeze(1)
            ...
            for i in range(0,max_n_clips,16):
                ...
                feats = self.act_net.act_base_1(clips_)
                feats = self.act_net.act_base_2(feats)
                feats = self.act_net.act_top(feats).mean(-1).mean(-1)
