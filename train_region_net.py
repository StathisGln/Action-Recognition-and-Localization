...
def validation(epoch, device, model, dataset_folder, sample_duration, spatial_transform, temporal_transform, boxes_file, splt_txt_path, cls2idx, batch_size, n_threads):
    ...
    for step, data  in enumerate(data_loader):
        ...
        for i in range(tubes.size(0)):
            ...
            n_elements = overlaps_max.gt(-1).nonzero().nelement()
            ...
            overlaps = bbox_overlaps_batch_3d(tubes[i].squeeze(0), gt_tubes_r_[i,:n_actions[i]].unsqueeze(0).type_as(tubes)).squeeze(0)
            max_overlaps, _ = torch.max(overlaps, 1)
            gt_max_overlaps, argmax = torch.max(overlaps, 0)
            ...
            tubes_over_ind = max_overlaps.view(overlaps.size(0)).gt(iou_thresh).nonzero().view(-1)

            if tubes_over_ind.nelement() > 0:

                overlaps_rois = overlaps_rois[tubes_over_ind]
                n_elem = tubes_over_ind.nelement()
...
if __name__ == '__main__':
    ...
    file = open('training_loss_ucf.txt','w')
    ...
    for epoch in range(epochs):
        ...
        if ( epoch + 1 ) % 5 == 0:
            torch.save(act_model.state_dict(), "action_net_ucf_2scores_mIoU.pwf".format(epoch+1))
    torch.save(act_model.state_dict(), "action_net_ucf_2scores_mIoU.pwf")
