...
class _Regression_Layer(nn.Module):
    ...
    """
    def __init__(self, din, sample_duration, n_classes):
        ...
        self.bbox_pred = nn.Linear(512,4*self.n_classes)
    ...
    def forward(self, base_feat, rois, gt_rois):
        ...
        bbox_pred = torch.zeros(base_feat.size(0),self.sample_duration,4*self.n_classes).type_as(base_feat)
        ...
        bbox_pred = bbox_pred.view(bbox_pred.size(0),self.sample_duration*4*self.n_classes).contiguous()
