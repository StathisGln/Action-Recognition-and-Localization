...
class Model(nn.Module):
    ...
    def forward(self,n_devs, dataset_folder, vid_names, clips, vid_id, boxes, mode, cls2idx, num_actions, num_frames, h_, w_, target):
        ...
        ### NMS then keep best 50
        if not self.training:

            f_tubes = final_tubes.new(batch_size, self.post_nms_tubes, max_n_frames, 4).zero_()

            for b in range(batch_size):
                
                fin_tubes =final_tubes[b].view(-1,num_frames*4).contiguous()
                fin_comb_scores =final_comb_scores[b]


                # keep_idx_i = nms_gpu.nms_gpu(torch.cat([fin_tubes,conn_scores.unsqueeze(1)], dim=1), 0.7)
                keep_idx_i = py_cpu_nms_tubes(torch.cat([fin_tubes,fin_comb_scores.unsqueeze(1)], dim=1).cpu().numpy(), 0.7)
                keep_idx_i = torch.Tensor(keep_idx_i).cuda().long().view(-1)
                ...
                print('keep_idx_i.numel() :',keep_idx_i.numel())
                f_tubes[b,:keep_idx_i.size(0)] = final_tubes[b,keep_idx_i]
        ...
        max_n_clips =  17
        features = torch.zeros(batch_size, final_tubes.size(1), max_n_clips, self.p_feat_size, self.sample_duration, self.POOLING_SIZE, self.POOLING_SIZE).type_as(clips)
        f_scores = torch.zeros(batch_size, self.post_nms_tubes, self.n_classes)
        for b in range(batch_size):

            tub_limits = torch.zeros(final_tubes.size(1),2)
            t = torch.arange(max_n_frames).unsqueeze(0).expand(final_tubes.size(1),max_n_frames).type_as(final_tubes)
            z = final_tubes[b].eq(0).all(dim=2)

            nonzero = z.eq(0).any(dim=1).nonzero().view(-1)

            tub_limits[:,0] = torch.min(t.masked_fill_(z,1000),dim=1)[0]
            tub_limits[:,1] = torch.max(t.masked_fill_(z,-1),dim=1)[0]+1

            first_frm = tub_limits[nonzero].min().int()
            last_frm = tub_limits[nonzero].max().int()

            frames_indices = torch.arange(first_frm, last_frm)
            
            indices = torch.arange(self.sample_duration)

            frm_indices = torch.arange(first_frm, last_frm-self.sample_duration+1, 2)
            max_n_clips = frm_indices.size(0)
            indices = indices.unsqueeze(0).expand(max_n_clips, self.sample_duration)
            indices = (indices + frm_indices.unsqueeze(1)).clamp_(max=num_frames[b].item()-1)

            all_scores = []
            for i in range(0,max_n_clips,16):

                frm_idx = indices[i:i+16,:]

                clips_ = clips[b,frame_indices].cuda()
                clips_ = clips_.contiguous().view(-1,self.sample_duration,3,self.sample_size,self.sample_size)
                clips_ = clips_.permute(0,2,1,3,4).contiguous()

                feats = self.cls_model_base(clips_)
                feats = self.cls_model_base_2(feats).mean(-1).mean(-1).view(feats.size(0),-1)
                scr = self.cls_top(feats)
                all_scores.append(scr)

            all_scores = torch.cat(all_scores, dim=0)

            for i in range(nonzero.numel()):

                start_fm = int((tub_limits[i,0].int() - first_frm.int())/2)
                end_fm =   int((tub_limits[i,1].int() - first_frm.int())/2)
                f_scores[b,i] = all_scores[start_fm:end_fm].mean(0)
        ...
        if self.training:
            ...
            cls_scr = F.softmax(f_scores, dim=2)
            ret_tubes = torch.zeros(batch_size, self.post_nms_tubes, ret_n_frames,4).type_as(final_tubes)
            ret_tubes[:,:final_tubes.size(1),:max_n_frames] = final_tubes

            ret_scr = torch.zeros(batch_size, self.post_nms_tubes, self.n_classes).type_as(final_tubes)
    ...
    def load_part_model(self, action_model_path=None, cls_path=None):
        ...
        if cls_path != None:
            ...
            cls_model = ACT_cls(self.classes, self.sample_duration)
            cls_model.create_architecture()
            ...
            self.cls_model_base = nn.Sequential(cls_model.base_net_1, cls_model.base_net_2 )
            self.cls_model_base_2 = nn.Sequential(cls_model.top_net)

            self.cls_top =  nn.Sequential(cls_model.cls)
        else:
