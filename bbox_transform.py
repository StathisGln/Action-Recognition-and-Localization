...
def clip_boxes_3d(boxes, im_shape, batch_size):
    ...
    for i in range(batch_size):
        ...
        boxes[i, :, 2::6]=boxes[i, :, 2::6].clamp_(min=0, max=im_shape[i, 2].item()-1)
        boxes[i, :, 3::6]=boxes[i, :, 3::6].clamp_(min=0, max=im_shape[i, 1].item()-1)
        boxes[i, :, 4::6]=boxes[i, :, 4::6].clamp_(min=0, max=im_shape[i, 0].item()-1)
        boxes[i, :, 5::6]=boxes[i, :, 5::6].clamp_(min=0, max=im_shape[i, 2].item()-1)
...
#         anchors_area = (anchors_boxes_x *
#                         anchors_boxes_y * anchors_boxes_z).view(batch_size, N, 1)
#         # print('anchors_area.shape :',anchors_area.shape)
